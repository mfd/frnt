...
class Experts extends PageBase {
	constructor() {
        ...
        this.gtmClickExpert = ::this.gtmClickExpert;
        ...
        this.resultsWrap.addEventListener('click',this.gtmClickExpert);
        
        EventsManager.on(Events.Filter.UPDATE, this.searchUpdated);
        EventsManager.on(Events.Filter.SEARCH, this.searchUpdated);
	}

    gtmClickExpert(event){
        if(event.target.classList.contains("expert")){
            let name = event.target.querySelector('div.small-title').innerHTML.toLowerCase();
        
            var dataLayer = window.dataLayer || [];
            dataLayer.push({
              'event':'engagement',
              'eventAction':'click on an expert profile',
              'eventLabel': name
            });
        }
    }
	...
	searchUpdated(data) {
        ...
        Ajax.load(
            ...
            (dataReceived) => {
                ...
                this.count = total;
                if(this.count == this.resultsWrap.querySelectorAll('.expert').length){
	...
	loadMoreArticles(event) {
        ...
        Ajax.load('//' + window.location.hostname + '/wp/wp-admin/admin-ajax.php', 
            (dataReceived) => {
                ...
                let total = response.total;
                this.count = total;
                if(this.count == this.resultsWrap.querySelectorAll('.expert').length || this.count == 0)
                    this.morphingContainer.style.display = "none";
...
// WEBPACK FOOTER //
// ../src/js/custom/views/experts/Experts.js