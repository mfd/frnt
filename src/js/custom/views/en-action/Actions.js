...
class Actions extends PageBase {
	constructor() {
		...
		// this.expert = this.view.querySelector(".expert--contact");
        ...
        // Params
        this.count = 0;
        this.postType = 'inaction-article';
        this.offset = 0;
        this.ppp = 12;
    ...
    **/
	...
	searchUpdated(data) {
        ...
        for(let i = 0; i < this.filterSelects.length; i++) {
            ...
            if(tax == 'expert'){
                this.filterSelects[i].selectedData = 'all';
                continue;
            }
            dataFilters[i] = {
                "data": data,
        ...
        Ajax.load(
            ...
            (dataReceived) => {
                let responseHtml = JSON.parse(dataReceived.response);

                // Insert results
                this.resultsWrap.innerHTML = responseHtml.html;

                // remove old anim rows + add new ones
                this.setAnimRows(this.resultsWrap.querySelectorAll('.anim--fade'), true);

                let total = responseHtml.total;
                this.count = total;
...
// WEBPACK FOOTER //
// ../src/js/custom/views/en-action/Actions.js